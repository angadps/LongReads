...
# Code to get the list of distances between consecutive snps across all chromosomes
for i in `seq 1 22`; do awk -F" " 'BEGIN{prev=0;} {dist=$2-prev;prev=$2;print dist;}' snps/snp_${i}.list; done | sort -g | uniq -c | awk '{print $2,"\t",$1}' > dist.list
gnuplot dist.p

# Code to get count of snps per read. Note that the count of overlapping snps will be lesser than the count obtained here.
for i in `cat simulated_positions.txt`; do echo $i | awk 'BEGIN{while((getline line<"snp_21.txt")>0) {i++;snps[i]=line;}} {for(i=1;i<=length(snps);i++) {for(j=0;j<4000;j++) {if($0+j>snps[i]) break; if($0+j==snps[i]) print $0,snps[i];}if(snps[i]>$0+j) break;}}' | wc -l; done | sort -g | uniq -c

# Awk one-liner for snps/reads and overlapping snps/pair of reads.
cat simulated_positions.txt | awk 'BEGIN{ct=0;while((getline line<"snp_21.txt")>0) {i++;snps[i]=line;}} {ct=0;for(i=1;i<=length(snps);i++) {for(j=0;j<4000;j++) {if($0+j>snps[i]) break; if($0+j==snps[i]) ct++;}if(snps[i]>$0+j) break;} print ct;}' | sort -g | uniq -c > snp1.ct

cat simulated_positions.txt | awk 'BEGIN{pos=10000000000000;while((getline line<"snp_21.txt")>0) {i++;snps[i]=line;}} {ct=0; for(i=1;i<=length(snps);i++) {for(j=0;j<4000;j++) {if($0+j>snps[i]||pos+j>snps[i]) break; if($0+j==snps[i]) ct++;}if(snps[i]>$0+j) break;} pos=$0; print ct;}' | sort -g | uniq -c > snp2.ct

# Code to identify good regions for testing
cat simulated_positions.txt | awk 'BEGIN{consec=0;pos=10000000000000;while((getline line<"snp_21.txt")>0) {i++;snps[i]=line;}} {ct=0; for(i=1;i<=length(snps);i++) {for(j=0;j<4000;j++) {if($0+j>snps[i]||pos+j>snps[i]) break; if($0+j==snps[i]) ct++;}if(snps[i]>$0+j) break;} if(ct>9) {consec++;} else { if(consec>100) print consec,"\t",pos,"\t",$0; consec=0;} pos=$0;}'
