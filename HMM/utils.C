...
char* SetCharVector(int maxElem)
// vectors starting at index 1
{
  char *v;
  v = new char[maxElem+1];
  assert(v != NULL);
...
char** SetCharMatrix(int maxRow, int maxCol)
// Sets memory for matrix from 1 to maxRow and from 1 to maxCol
{
  int i;
  char **m;

  m = new char*[maxRow];// array of pointers
  assert(m != NULL);
  m[0] = new char[maxRow*maxCol];// assign space for elements
  assert(m[0] != NULL);
...
double AvgElemDiff(double **m, double **n, int maxRow, int maxCol)
// Find maximum of absolute values of differences between elements of 2 matrices
{
  int i, j;
  ...
  return avgDiff/(maxRow*maxCol);
}

//===============================================================================

void UnLogArray(double **v, int maxRow, int maxCol)
{
  int i, j;
  for(i=1;i<=maxRow;i++){
    for( j=1;j<=maxCol;j++){
      v[i][j] = pow(2.7183,v[i][j]);
    }
  }
}
...
void AssignMatrix(double **m, double prob, int maxRow, int maxCol)
{
  int i, j;
  double sum;
...
void CopyMatrix(double **m, double **n, int maxRow, int maxCol)
{
  int i, j;
  double sum;
...
void InitEmissionRows(double **m, int maxRow, int maxCol)
  ...
  //  a diagonal assignment matrix.
{
  ...
  double std = ((double)maxCol)/10.0;

  for(i=1;i<=maxRow;i++){
    for(k=1;k<=maxCol;k++){
      m[i][k] = 1.0/((double)maxCol);
    }
  }
/*
  for(i=1;i<=maxRow;i++){
...
*/
  m[1][1] = 1.0 - 1.0/((double)maxCol);
  m[2][1] = 1.0/((double)maxCol);
  //m[1][1] = 1.0;
  //m[2][1] = 0.0;
  // Renormalize row by row:

  // REVISIT: Consider skipping normalization
  NormalizeAllRows(m, maxRow, maxCol);
...
// Assumes maxCol == 2
void LogNormalize(double *row, int maxCol)
{
  int k;
  double temp[3];
  ...
  temp[1] = 1.0 / (1.0 + pow(2.7183,row[2]-row[1]));
  temp[2] = 1.0 / (1.0 + pow(2.7183,row[1]-row[2]));
