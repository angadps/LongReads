...
void CFlexibleObsProb::BWSum(double *gamma, int symbol)
{
	int i;
	
	for(i=1; i<=mDimension; i++){
		mComponentProb[i]->BWSum(gamma, symbol);
	}
}
...
double CFlexibleObsProb::EndLogIter()
// Finish Baum-Welch iteration; return magnitude of largest change
{
	...
	for(i=1;i<= mDimension; i++){
		componentMaxDiff = mComponentProb[i]->EndLogIter();
...
void CFlexibleObsProb::UnLog()
{
	int i;
	
	for(i=1;i<= mDimension; i++){
		mComponentProb[i]->UnLog();
	}
}
