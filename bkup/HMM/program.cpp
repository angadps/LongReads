...
void read_snp_file(FILE *snp_file, long snp_end)
{
	...
	while(true) {
		...
		if(vcf_line[4].find(',')==string::npos) { // && vcf_line[9].find("0/1")!=string::npos) {
		{ //if(vcf_line[7].find("DP=2;")==string::npos && vcf_line[7].find("DP=3;")==string::npos && vcf_line[7].find("DP=4;")==string::npos && vcf_line[7].find("DP=5;")==string::npos) {
			...
			//SNP *snp = new SNP(vcf_line[0], pos, vcf_line[3].c_str()[0], vcf_line[4].c_str()[0], gl3, known, atoi(format[4].c_str()));
			SNP *snp = new SNP(vcf_line[0], pos, vcf_line[3].c_str()[0], vcf_line[4].c_str()[0], gl3, known, atof(vcf_line[5].c_str()));
...
#endif
						int ndels = get_deletion_count(cigar,snp_pos-start);
#ifdef FULLDEBUG
...
cout << "Nbsymbols = " << nbSymbols << endl;
	...
	learnedHMM->LearnEmissionBaumWelch(obsSeq, distanceOutput, snp_start, snp_end);
	...
	for(vector<SNP*>::iterator snp_it = snp_list.begin(); snp_it != snp_list.end(); snp_it++) { // check for snps in vector
		...
		double known_prior[3], novel_prior[3], prior[3], sam_post[3], hmm_post[3];
		...
		stateOutput << (*snp_it)->GetChr() << "\t" << (*snp_it)->GetPos() << "\t" << (*snp_it)->GetKnown() << "\t" << gp << "\t" << hmm_post[0] << "\t" << hmm_post[1] << "\t" << hmm_post[2] << "\t" << sam_post[0] << "\t" << sam_post[1] << "\t" << sam_post[2] << "\t" << sam_post[1]/sam_post[0] << "\t" << hmm_post[1]/hmm_post[0] << endl;
...
int main(int argc, char **argv)
{
	...
	const char *base_name="/ifs/scratch/c2b2/ys_lab/aps2157/Haplotype/HMM/output/output.21";
