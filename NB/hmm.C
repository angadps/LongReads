...
#ifdef DEBUG
cout << sp->GetPos() << " " << sp->GetRef() << " " << sp->GetAlt() << " " << (*rd).GetAllele(known_index[count]);
#endif
					// REVISIT: Implement
...
cout << "Happrob[2] = " << haprob[2] << endl;
			...
			rd->assignHaplotype(hap, happrob, 0);
			happos = rd->GetPos();
		}
...
#endif
		// NOTE!!!!!!!!!!!!!!!
		// Since we're using log scales here, we reverse the values
		norm = emission_list[1] + emission_list[2];
		if(norm!=0.0) {
...
//prevalence=0.2;
		...
		if(hap==1) {
                	if(all == ref) {
                        	...
                        	//probs1[0] *= ((haprob) * (1.0-qualscore));
                        	//probs1[1] *= (prevalence*haprob*qualscore);
                	...
                	if(all == ref) {
                        	...
                        	//probs2[0] *= ((haprob) * (1.0-qualscore));
                        	//probs2[1] *= (prevalence*haprob*qualscore);
...
//        if(real_count==0 || (ref_ct>=8&&alt_ct<2) || (ref_ct>=16&&alt_ct<3) || alt_ct<4 || ref_ct<alt_ct || (alt1_ct>=3&&alt2_ct>=3) || (ref1_ct<=3||ref2_ct<=3)) {
//                probs[0] = 1.0;
//		probs[1] = probs[2] = 0.0;
	//if(real_count==0 || alt_ct<1 || ref_ct < 3 || (ref1_ct<2||ref2_ct<2) || alt1_ct>(int)(ref1_ct*1.5)||alt2_ct>(int)(ref2_ct*1.5)) {
...
prevelance = (double)alt_ct/(double)(ref_ct+alt_ct);
