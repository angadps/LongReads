...
then
	...
	qsub -sync y -t 1-22 -l mem=6G,time=4:: ./simulate_read.sh $snprate $errrate $coverage $readlen $region ${read_base}
	echo simulation complete

	let "qtime=3*$coverage"
	qsub -sync y -t 1-22 -l mem=8G,time=16:: ./long_read_map.sh ${read_base} $region
	...
	do
		...
		qsub -sync y -t 1-22 -l mem=2G,time=32:: ./program.sh ${read_base} ${vcf_base} $region $iteration ${output_base} $pre $errrate
	...
	let "qmem=400*$coverage"
	qsub -sync y -l mem=4G,time=4:: ./simulate_read.sh $snprate $errrate $coverage $readlen $region ${read_base}
	echo simulation complete

	let "qtime=3*$coverage"
	qsub -sync y -l mem=8G,time=16:: ./long_read_map.sh ${read_base} $region
	echo alignment complete
	...
	do
		output_base=${base}/HMM/output/${pre}_${iteration}
		qsub -sync y -l mem=2G,time=32:: ./program.sh ${read_base} ${vcf_base} $region $iteration ${output_base} $pre $errrate
		echo HMM complete
		qsub -l mem=1G,time=2:: ./analysis.sh ${output_base}_${region} ${read_base}_${region}
	done
fi
