...
//      CHMM(char* hmmFileName);
	...
	double ForwardEmission(double **alpha, double *scale, CObs **obs, long T, boolean doLog);
	void BackwardEmission(double **beta, double *scale, CObs **obs, long T);
	double ForwardLogEmission(double **alpha, double *scale, CObs **obs, long T, boolean doLog);
	double BackwardLogEmission(double **beta, double *scale, CObs **obs, long T);
	...
	double BaumWelchCoreEmission(CObs **obs, long T, ostream &distanceOutput, double *gamma, double **xi, boolean doLog, long snp_start, long snp_end);
        double IterEmissionBaumWelch(CObsSeq *obsSeq, ostream &distanceOutput, double *gamma, double **xi, long snp_start, long snp_end);
	void LearnEmissionBaumWelch(CObsSeq *obsSeq, ostream &distanceOutput, long snp_start, long snp_end);
	...
	void GetCommonSnpList(CObs **obs, SNP** reads_snp_list, SNP** known_snp_list, int *common_snp_count, int *known_snp_count, int *common_index, int *known_index, int t);
