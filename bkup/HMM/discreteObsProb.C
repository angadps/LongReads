...
CDiscreteObsProb::CDiscreteObsProb(double **mat, int nbSymbols, int nbStates)
{
	mM = nbSymbols;
	mN = nbStates;
...
CDiscreteObsProb::CDiscreteObsProb(int nbSymbols, int nbStates)
{
	...
	mNextB = SetMatrix(mN, mM);
	mLogB = SetMatrix(mN, mM);
	InitEmissionRows(mThisB, mN, mM);// Do not randomize this one
...
void CDiscreteObsProb::BWSum(double *gamma, int symbol)
{
  int i;
  
  for (i = 1; i <= mN; i++) {
      mSum[i][symbol] += gamma[i];
  }
}
...
void CDiscreteObsProb::UnLog()
{
      UnLogArray(mSum, mN, mM);// copy new result into mThisB
}

double CDiscreteObsProb::EndLogIter()
{
      double maxDiff;
		
      CopyArray(mSum, mNextB, mN, mM); // Copies mSum to mNextB
      NonZeroNormalizeAllRows(mNextB, mN, mM);// check if mBWSum is really not needed
      maxDiff = AvgElemDiff(mNextB, mThisB, mN, mM);// compare previous and new B result
